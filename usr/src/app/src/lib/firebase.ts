...
export async function updateProductStatus(
  ...
  try {
    ...
    if (status === 'approved') {
      ...
      await sendProductRejectedEmail({ seller, product, reason: rejectionReason || 'No reason provided.' });
    }
  } catch (error) {
    console.error("Failed to send product status email notification:", error);
    // We don't throw an error here because the primary action (updating status) succeeded.
    // We just log it.
  }
}
