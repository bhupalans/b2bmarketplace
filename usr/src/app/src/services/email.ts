...
export async function sendProductApprovedEmail({ seller, product }: { seller: User, product: Product }) {
    ...
    try {
        await resend.emails.send({
            ...
            html: `
                <div style="font-family: sans-serif; line-height: 1.5;">
                    ...
                    <p>Your product submission, <strong>${product.title}</strong>, has been reviewed and approved by our team.</p>
                    <p>It is now live on the marketplace and visible to buyers.</p>
                    <p>
        ...
        console.log(`Product approval email sent to ${seller.email}`);
    } catch (error) {
        console.error(`Failed to send product approval email to ${seller.email}:`, error);
    }
}
...
export async function sendProductRejectedEmail({ seller, product, reason }: { seller: User, product: Product, reason: string }) {
    ...
    const myProductsUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/my-products`;

    try {
        await resend.emails.send({
            from: fromAddress,
            to: seller.email,
            subject: `Action required for your product: "${product.title}"`,
            html: `
                <div style="font-family: sans-serif; line-height: 1.5;">
                    <h1>Hi ${seller.name},</h1>
                    <p>There was an issue with your product submission for <strong>${product.title}</strong>, and it could not be approved at this time.</p>
                    <hr>
                    <p style="color: #555;">Admin Feedback:</p>
                    <p style="font-style: italic; border-left: 3px solid #dc3545; padding-left: 10px;">"${reason}"</p>
                    <hr>
                    <p>Please review the feedback, update your product listing, and re-submit it for approval.</p>
                     <p>
                        <a href="${myProductsUrl}" style="display: inline-block; padding: 10px 15px; background-color: #007bff; color: #ffffff; text-decoration: none; border-radius: 5px;">
                            Go to My Products
                        </a>
                    </p>
                </div>
        ...
        console.log(`Product rejection email sent to ${seller.email}`);
    } catch (error) {
        console.error(`Failed to send product rejection email to ${seller.email}:`, error);
    }
}
