...
// Keep priceObject available for display (Originally ...)
const priceObject = offer.price || { baseAmount: offer.pricePerUnit || 0, baseCurrency: 'USD' };

let unitPrice = 0;
let totalPrice = 0;

// If offer is accepted and frozen pricing exists → use frozen USD
if (offer.status === 'accepted' && offer.pricing?.usd?.total && isRatesLoaded) {
  
  const usdTotal = offer.pricing.usd.total;

  // Convert USD → selected currency
  const usdRate = rates[currency] || 1; // USD base
  totalPrice = usdTotal * usdRate;

  unitPrice = totalPrice / offer.quantity;

} else {
  // Before acceptance → use dynamic conversion
  unitPrice = isRatesLoaded ? convertPrice(priceObject, currency, rates) : priceObject.baseAmount;
  totalPrice = unitPrice * offer.quantity;
}
